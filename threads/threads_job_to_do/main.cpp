...
sleep(1);
    for (i = 0; i < TCOUNT; i++) {
        ...
        if (!data->jobToBeDone.empty()) {
            ...
            printf("inc_count(): thread %ld, popped element = %f  Job done. ",
        ...
        if (data->jobToBeDone.empty()) {
            ...
            printf("inc_count(): thread %ld, stack size = %lu, unlocking mutex\n",
...
void* watch_count(void* threadarg) {
  ...
  */
  ...
  //while (count < COUNT_LIMIT) {
    printf("watch_count(): thread %ld stack size= %lu. Going into wait...\n", syscall(SYS_gettid), stackSize);
    pthread_cond_wait(&count_threshod_cv, &count_mutex);
    printf("watch_count(): thread %ld Condition signal received. stack size= %lu\n", syscall(SYS_gettid), data->jobToBeDone.size());
    printf("watch_count(): thread %ld Updating the value of stack...\n", syscall(SYS_gettid));
//    for(int i=0; i<10; ++i) {
//           data->jobToBeDone.push(47);
//       }
    printf("watch_count(): maybe later. Thank you very much.\n", syscall(SYS_gettid));
    printf("watch_count(): thread %ld stack size now = %lu.\n", syscall(SYS_gettid), data->jobToBeDone.size());
    //}
...
int main(int argc, char *argv[]) {
  ...
  printf ("Main(): Waited and joined with %d threads. Final size of stack with tasks = %lu. Done.\n", 
